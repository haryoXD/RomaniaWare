...
local themes = {
	selected = "Default",
	Default = {
		textForeground = Color3.fromRGB(247, 109, 109),
		...
		toggleEnabled = Color3.fromRGB(247, 109, 109),
...
if not isfolder("Hyperhook\\Themes") then
	pcall(makefolder, "Hyperhook")
	pcall(makefolder, "Hyperhook\\Themes")
end
...
for i, v in next, themes do
	if i ~= "selected" then
		writefile("Hyperhook\\Themes\\" .. i .. ".json", jsonEncodeTheme(v))
	end
end
...
function library.new(gameName)
	local lib = setmetatable({
		_name = gameName,
		_gui = create("ScreenGui", { Name = "hyperhook", ZIndexBehavior = Enum.ZIndexBehavior.Sibling }, {
			create("Frame", { Name = "main", BackgroundColor3 = "theme.mainBackground", ClipsDescendants = true, Position = UDim2.new(0, 100, 0, 100), Size = UDim2.new(0, 535, 0, 380) }, {
				...
				create("Frame", { Name = "top", BackgroundColor3 = "theme.topBackground", Size = UDim2.new(1, 0, 0, 34), ZIndex = 2 }, {
					create("TextLabel", { Name = "title", BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0, 34, 0, 0), Size = UDim2.new(1, -68, 1, 0), Text = "Hyperhook PREMIUM" | " .. gameName, TextColor3 = "theme.textForeground", TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left }),
				...
				create("TextLabel", { Name = "site", AnchorPoint =  Vector2.new(0.5, 1), BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0.5, 0, 1, 0), Size = UDim2.new(1, -16, 0, 26), Text = "https://discord.gg/JFqXmgpC4n", TextColor3 = "theme.textForeground", TextSize = 12, TextXAlignment = Enum.TextXAlignment.Right }),
...
function library:GetConfigs()
	local configs = {}
	if isfolder("Hyperhook\\Configs\\" .. self._name) then
		for i, v in next, listfiles("Hyperhook\\Configs\\" .. self._name) do
...
function library:LoadConfig(name)
	local filePath = "Hyperhook\\Configs\\" .. self._name .. "\\" .. name .. ".json"
	if not (isfolder("Hyperhook\\Configs\\" .. self._name) and isfile(filePath)) then
		return
	end
...
function library:SaveConfig(name)
	if not isfolder("Hyperhook\\Configs\\" .. self._name) then
		pcall(makefolder, "Hyperhook")
		pcall(makefolder, "Hyperhook\\Configs")
		pcall(makefolder, "Hyperhook\\Configs\\" .. self._name)
	end
	local config = {}
	for _, tab in next, self._tabs do
		for _, panel in next, tab._panels do
	...
	writefile("Hyperhook\\Configs\\" .. self._name .. "\\" .. name .. ".json", httpService:JSONEncode(config))
end
...
function library:GetThemes()
	...
	for i, v in next, listfiles("Hyperhook\\Themes") do
		local split = select(-1, unpack(v:split("\\")))
		if split:find(".json") then
...
function library:LoadTheme(name)
	local filePath = "Hyperhook\\Themes\\" .. name .. ".json"
	if not (isfolder("Hyperhook\\Themes") and isfile(filePath)) then
		print("noes")
		return
	end
...
function library:AddSettings()
	...
	credits:AddClipboardLabel("Zelt#2710 - Owner, Scripter", "https://discord.gg/JFqXmgpC4n")
	credits:AddClipboardLabel("Kieran - UI", "https://discord.gg/QYH4F7ks7m")
...
function library:Notify(text, callback, options)
	...
	local frame = create("Frame", { Name = "notification", AnchorPoint = Vector2.new(1, 1), BackgroundColor3 = "theme.panelItemBackground", ClipsDescendants = true, Parent = self._gui.notifs, Position = UDim2.new(1, 300, 1, -30), Size = UDim2.new(0, 280, 0, sizeY + 34) }, {
		create("TextLabel", { Name = "title", BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0, 10, 0, 0), Size = UDim2.new(1, -66, 0, 30), Text = "Hyperhook Notification", TextColor3 = "theme.textForeground", TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left }),
